...
#   python scripts/build_cache.py   # falls data/data_complete.csv noch fehlt
#
# Start (Terminal):
...
REGS = ["NO", "FIT", "MPM_EX", "CFD", "CFD_MONTH_EST", "CFD_YEAR_EST", "CFD_YEAR_PREV", "CFD_CAPABILITY", "CFD_FINANCIAL"] #"MPM", FIT_PREMIUM
# Speichern der PNGs?
SAVE_PNG = True  # auf False setzen, wenn nur Anzeige gewünscht

# Deltas für Sensitivitäten (€/MWh)
DELTAS = (-80,-70,-60,-50,-40,-30,-20,-10,0,10,20,30,40,50,60,70,80)
...
def main():
    ...
    plot_strategies_breakdown_ID(
        details_all, freq="MS",
        title="Erlösstruktur je Regime – 2024"
    )
    if SAVE_PNG:
    ...
    plot_support_by_regime(
        details_all, freq="MS",
        title="Förderhöhe je Regime – 2024", stacked=True
    )
    if SAVE_PNG:
