...
#                               deltas=range(-50, 51, 10), weight="Act_raw_MW")
...
from .config import (
    FORECAST_ID_COL,
    ID_PRICE_COL,
    MV_REAL_COL,   # realisierte Monats-Marktwerte
    REFERENCE_OUTPUT_COL,
    FINANCIAL_CFD_HOURLY_EUR,
    START_DATE,
    END_DATE,
    P_FIT,
    CFD_K,
...
def conformity_sensitivity(
    ...
    # CFD_MONTH_EST: K sweep
    base_k = float(CFD_K)
    for d in deltas:
        k = base_k + d
        det_cfd_month_est, _ = reduced_strategies(
            df=df, regime="CFD_MONTH_EST", cfd_strike=k,
            start_date=start, end_date=end,
            forecast_id_col=FORECAST_ID_COL, id_price_col=ID_PRICE_COL,
            market_value_est_col=MV_EST_COL,
        )
        idx = market_conformity_index(det_ref, det_cfd_month_est, weight_col=weight_col)
        rows.append({"Regime":"CFD_MONTH_EST", "Delta": d, "BaseValue": base_k, "TestValue": k, "MarketSimilarityIndex": idx})

    # CFD_YEAR_EST: K sweep (Referenz = Jahres-MV-Schätzer)
    base_k = float(CFD_K)
    for d in deltas:
        k = base_k + d
        det_cfd_year_est, _ = reduced_strategies(
            df=df, regime="CFD_YEAR_EST", cfd_strike=k,
            start_date=start, end_date=end,
            forecast_id_col=FORECAST_ID_COL, id_price_col=ID_PRICE_COL,
            market_value_year_est_col=MV_YEAR_EST_COL,
        )
        idx = market_conformity_index(det_ref, det_cfd_year_est, weight_col=weight_col)
        rows.append({"Regime":"CFD_YEAR_EST", "Delta": d, "BaseValue": base_k, "TestValue": k,
                     "MarketSimilarityIndex": idx})

    # CFD_YEAR_PREV: K sweep (Referenz = Jahres-MV des Vorjahres, gemappt)
    base_k = float(CFD_K)
    for d in deltas:
        k = base_k + d
        det_cfd_year_prev, _ = reduced_strategies(
            df=df, regime="CFD_YEAR_PREV", cfd_strike=k,
            start_date=start, end_date=end,
            forecast_id_col=FORECAST_ID_COL, id_price_col=ID_PRICE_COL,
            market_value_year_prev_col=MV_YEAR_PREV_COL,
        )
        idx = market_conformity_index(det_ref, det_cfd_year_prev, weight_col=weight_col)
        rows.append({"Regime":"CFD_YEAR_PREV", "Delta": d, "BaseValue": base_k, "TestValue": k,
                     "MarketSimilarityIndex": idx})

    # CFD_CAPABILITY: K-Sweep (entscheidungsneutral, aber für Vollständigkeit)
    base_k = float(CFD_K)
    for d in deltas:
        k = base_k + d
        det_capcfd, _ = reduced_strategies(
            df=df, regime="CFD_CAPABILITY", cfd_strike=k,
            start_date=start, end_date=end,
            forecast_id_col=FORECAST_ID_COL, id_price_col=ID_PRICE_COL
        )
        idx = market_conformity_index(det_ref, det_capcfd, weight_col=weight_col)
        rows.append({"Regime":"CFD_CAPABILITY", "Delta": d, "BaseValue": base_k, "TestValue": k, "MarketSimilarityIndex": idx})

    # CFD_FINANCIAL: Flat-Sweep (Entscheidung = NO → Index meist konstant)
    base_flat = float(FINANCIAL_CFD_HOURLY_EUR)
    for d in deltas:
        flat = base_flat + d
        det_fin, _ = reduced_strategies(
            df=df, regime="CFD_FINANCIAL",
            financial_cfd_hourly_eur=flat,
            start_date=start, end_date=end,
            forecast_id_col=FORECAST_ID_COL, id_price_col=ID_PRICE_COL,
            # Referenzgenerator-Output (falls du ihn anders benennst → Config anpassen)
            reference_output_col=REFERENCE_OUTPUT_COL,
        )
        idx = market_conformity_index(det_ref, det_fin, weight_col=weight_col)
        rows.append({"Regime":"CFD_FINANCIAL", "Delta": d, "BaseValue": base_flat, "TestValue": flat, "MarketSimilarityIndex": idx})
    ...
    for d in deltas:
        ...
        det_mpm, _ = reduced_strategies(
            ...
            market_value_col=MV_REAL_COL, market_value_est_col=MV_EST_COL,
            start_date=start, end_date=end,
            forecast_id_col=FORECAST_ID_COL, id_price_col=ID_PRICE_COL
        )
...
def financial_sensitivity(
    ...
    deltas: Iterable[int] = (-80,-70,-60,-50,-40,-30,-20,-10,0,10,20,30,40,50,60,70, 80),
    ) -> pd.DataFrame:
    rows = []
    ...
    for d in deltas:
        ...
        rows.append(_row_from_totals("CFD_MONTH_EST", d, base_k, k, totals))

    # CFD_YEAR_EST – K-Sweep
    base_k = float(CFD_K)
    for d in deltas:
        k = base_k + d
        _, totals = reduced_strategies(
        ...
        rows.append(_row_from_totals("CFD_YEAR_EST", d, base_k, k, totals))

    # CFD_YEAR_PREV – K-Sweep
    base_k = float(CFD_K)
    for d in deltas:
        k = base_k + d
        _, totals = reduced_strategies(
        ...
        rows.append(_row_from_totals("CFD_YEAR_PREV", d, base_k, k, totals))

    # CFD_CAPABILITY – K-Sweep
    base_k = float(CFD_K)
    for d in deltas:
        k = base_k + d
        _, totals = reduced_strategies(
        ...
        rows.append(_row_from_totals("CFD_CAPABILITY", d, base_k, k, totals))

    # CFD_FINANCIAL – Flat-Sweep (€/h)
    base_flat = float(FINANCIAL_CFD_HOURLY_EUR)
    for d in deltas:
        flat = base_flat + d
        _, totals = reduced_strategies(
        ...
        rows.append(_row_from_totals("CFD_FINANCIAL", d, base_flat, flat, totals))
