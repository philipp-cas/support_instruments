...
# Hinweis: Der eigentliche Rohdaten-Aufbau erfolgt extern (build_data_from_sources in data_store).
# Hier werden die neuen Spalten erzeugt und Schätzer berechnet.
# from src.data_import import build_data_from_sources  # nicht mehr direkt benötigt
...
# DATA_DIR kommt jetzt als Funktionsargument herein (siehe process_data)
ASSET_SHARE = 0.02  # Wir modellieren ein Asset, das 2 % der installierten TransnetBW-Kapazität repräsentiert
YEARS = (2021, 2022, 2023, 2024, 2025)  # bleibt rein dokumentarisch hier erhalten


def process_data(data: pd.DataFrame, DATA_DIR: Path) -> pd.DataFrame:
    """
    Nimmt den aus Quellen gebauten 15-Minuten-DataFrame entgegen und erzeugt alle
    abgeleiteten Spalten (Assets/Referenz sowie Monats-/Jahres-Schätzer) – exakt
    nach der bisherigen Logik. Das Speichern erfolgt extern.
    """
    ...
    # (Die Daten sind jetzt bereits als Argument `data` übergeben.)
    ...
    # Check: Referenz-Output-Spalte existiert
    expected = "asset_ref_Wind Onshore_act"
    if expected not in data.columns:
        missing = [c for c in [
            "BE_Wind Onshore_act","BE_Wind Onshore_capacity","TransnetBW_Wind Onshore_capacity",
        ] if c not in data.columns]
        raise KeyError(
            f"'{expected}' konnte nicht erzeugt werden. " f"Fehlende Eingangs-Spalten: {missing}. "
 )            
    ...
    #    - Windprofil des Restes ≈ typisches stündliches Tagesprofil der letzten x Tage
    ...
    # =============================================================================================
    # 5) Jährlicher realiserter Marktwert
    # =============================================================================================
 
    #  Vorjahres-Marktwert auf QH-Raster: Wind-gewichteter Real-MW des Vorjahres ---
    df_h = data.resample("h").mean(numeric_only=True)

    # Realisierter Jahres-Marktwert (windgewichtet) je Kalenderjahr
    num_y = (df_h["da_price"] * df_h["DE_Wind Onshore_hochrechnung"]).groupby(df_h.index.year).sum()
    den_y = df_h["DE_Wind Onshore_hochrechnung"].groupby(df_h.index.year).sum()
    mv_year_real = (num_y / den_y).replace([np.inf, -np.inf], np.nan)

    mv_real_series = pd.Series(
    [mv_year_real.get(ts.year, np.nan) for ts in data.index],
    index=data.index,
    name="Wind Onshore_marketvalue_year",
    dtype="float64",
    )
    data["Wind Onshore_marketvalue_year"] = mv_real_series

    # Mapping: Jahr t → MW(real) von Jahr t-1
    mv_prev_map = {y+1: v for y, v in mv_year_real.dropna().items()}

    # Auf 15-Minuten-Raster mappen (konstant pro Jahr)
    mv_prev_series = pd.Series(
        [mv_prev_map.get(ts.year, np.nan) for ts in data.index],
        index=data.index,
        name="Wind Onshore_marketvalue_year_prev"
        )
    
    data["Wind Onshore_marketvalue_year_prev"] = mv_prev_series
    ...
    # (Speichern wird extern in data_store.save_data() erledigt)
    ...
    # print(" -", out_csv)
    print(" - (Speichern extern via data_store.save_data)")
