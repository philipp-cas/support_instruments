...
from .config import (
    ...
    FORECAST_ID_COL, ID_PRICE_COL, MV_REAL_COL, MV_EST_COL,
    REFERENCE_OUTPUT_COL, FINANCIAL_CFD_HOURLY_EUR, MV_YEAR_EST_COL, MV_YEAR_PREV_COL,
    # Parameter
...
def _regime_extras(regime: str) -> dict:
    ...
    if r == "CFD_MONTH_EST":
        # CFD_MONTH_EST braucht den MV-Estimator; Strike kommt aus CFD_K
        return {
            "cfd_strike": CFD_K,
    ...
    if r == "CFD_CAPABILITY":
        # Capability-CfD: Strike wird benötigt, Rest ist wie NO/QUANT bzgl. Dispatch
        return {
            "cfd_strike": CFD_K,
        }
    if r == "CFD_FINANCIAL":
        # Pauschale (€/h) + Referenzgenerator-Ausgabe-Spalte
        return {
            "financial_cfd_hourly_eur": FINANCIAL_CFD_HOURLY_EUR,
            "reference_output_col": REFERENCE_OUTPUT_COL,
        }
    
    if r == "CFD_YEAR_EST":
    # nutzt Standardspaltenname aus dem Core, d. h. Parameter ist optional
        return {"cfd_strike": CFD_K, "market_value_year_est_col": MV_YEAR_EST_COL}

    if r == "CFD_YEAR_PREV":
         return {"cfd_strike": CFD_K, "market_value_year_prev_col": MV_YEAR_PREV_COL}
